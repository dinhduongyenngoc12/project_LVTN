...
export type AdminDashboardSummary = DashboardSummary & {
    ...
    devicesWithoutThreshold: number;
};
...
export type AdminDashboardData = {
    ...
    devicesWithoutThreshold: DashboardDevice[];
};
...
export function buildAdminDashboardData(devices: DashboardDevice[], totalUsers = 0): AdminDashboardData {
    ...
    const topConsumers = [...devices]
        .sort((left, right) => (right.latestPower ?? 0) - (left.latestPower ?? 0))
        .slice(0, 5);
    const devicesWithoutThreshold = devices.filter((device) => device.maxPower === null);
    ...
    return {
        summary: {
            ...
            devicesWithoutThreshold: devicesWithoutThreshold.length,
        },
        userSummaries,
        alertDevices,
        topConsumers,
        devicesWithoutThreshold,
    };
}
